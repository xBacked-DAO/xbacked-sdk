...
class VaultsClient extends Account_1.Account {
    constructor(params) {
        super(params);
    }
    /**
    ...
    depositCollateral(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const ctc = this.reachAccount.contract(params.vault.backend, params.vault.id);
            const put = ctc.a.VaultOwner;
            const res = yield put.depositCollateral((0, utils_1.convertToMicroUnits)(params.amount, params.vault.asaVault ? params.vault.asaVault.decimals : undefined));
            return res;
        });
    }
    /**
    ...
    withdrawCollateral(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const res = yield put.withdrawCollateral({
                collateralToWithdraw: (0, utils_1.convertToMicroUnits)(params.amount, params.vault.asaVault ? params.vault.asaVault.decimals : undefined),
                minPrice: (0, utils_1.convertToMicroUnits)(params.minimumPrice),
                maxPrice: (0, utils_1.convertToMicroUnits)(params.maximumPrice),
            });
            return res;
    ...
    getVaultAnalytics(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (Object.keys(utils_1.VAULTS[this.network]).includes(params.vault.name)) {
                const vaultDetails = utils_1.VAULTS[this.network][params.vault.name];
                const collateralAssetId = vaultDetails.assetId ? vaultDetails.assetId : 0;
                const vaultAddress = algosdk_1.default.getApplicationAddress(vaultDetails.vaultId);
                ...
                const totalValueLocked = (collateralBalance * collateralPrice) / (Math.pow(10, (vaultDetails.assetDecimals || 6)));
                const accounts = yield (0, utils_1.getAllAccounts)(
                // application ID
                vaultDetails.vaultId, params.indexer, [], "");
                const vaultData = accounts.map((vaultAccount) => {
                    try {
                        return this.getUserInfo({ address: (0, utils_1.addrFromBox)(vaultAccount), vault: params.vault });
                    }
                    catch (error) {
                        return { error };
                    }
                });
                const resolvedVaultData = yield Promise.all(vaultData);
                console.log({ resolvedVaultData });
                const totalVaultDebt = resolvedVaultData.reduce((prev, next) => prev + (next.error ? 0 : next.vaultDebt), 0);
                const totalVaultsGotten = resolvedVaultData.filter((el) => el.error == undefined).length;
    ...
    createVault(params) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const res = yield put.createVault({
                initialCollateral: (0, utils_1.convertToMicroUnits)(params.collateral, params.vault.asaVault ? params.vault.asaVault.decimals : undefined),
